...
// A map of "trigger option ID" -> { target_group_id, allowed_option_ids }
// built by PHP from whatever compatibility rules an Administrator has
// configured. An empty object here (the default) means no rules exist
// anywhere, so every option stays available everywhere, unchanged
// from before this feature existed.
var compatibilityRules = {};

try {
compatibilityRules = JSON.parse( wizard.dataset.compatibilityRules || '{}' );
} catch ( e ) {
compatibilityRules = {};
}
...
/**
 * Looks at every selection the customer has made so far and works
 * out whether any of them trigger a compatibility rule targeting
 * the group given by targetGroupId.
 *
 * Returns null if no active selection restricts this group (meaning
 * every option in it should stay available). Otherwise returns a
 * small helper object with a has(optionId) method, true only for
 * option IDs explicitly allowed by the matching rule(s).
 */
function getAllowedOptionIdsForGroup( targetGroupId ) {
...
var rule = compatibilityRules[ selection.optionId ];

if ( ! rule ) {
continue;
}

if ( String( rule.target_group_id ) !== String( targetGroupId ) ) {
continue;
}
...
var allowedIds = rule.allowed_option_ids || [];
...
return !! allowed[ String( optionId ) ];
}
};
}

/**
 * Re-applies every compatibility rule to every option step, based
 * on the customer's current selections. For each group: hides any
 * tile (or disables any dropdown option) that a currently-active
 * rule does not allow, and clears out any existing selection in
 * that group that is no longer allowed. Safe to call after any
 * selection changes, and safe to call on steps that are not
 * currently visible on screen.
 */
function refreshCompatibilityRestrictions() {

for ( var i = 0; i < totalOptionSteps; i++ ) {

var step        = steps[ i ];
var groupId     = step.dataset.groupId;
var allowedIds  = getAllowedOptionIdsForGroup( groupId );
...
var isAllowed = ( allowedIds === null ) || allowedIds.has( tile.dataset.optionId );

tile.style.display = isAllowed ? '' : 'none';
} );

if ( selections[ i ] && allowedIds !== null && ! allowedIds.has( selections[ i ].optionId ) ) {
...
var option = dropdown.options[ o ];

if ( ! option.value ) {
continue;
}

var isOptionAllowed = ( allowedIds === null ) || allowedIds.has( option.value );

option.hidden   = ! isOptionAllowed;
option.disabled = ! isOptionAllowed;
}
