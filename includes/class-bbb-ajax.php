 ...
 * returns true (Settings > BBB Pricing, off by default), this class
 * also recalculates the build's estimated total entirely server-side,
 * by summing the price_delta of each submitted option_id straight
 * from the database - never from anything the browser sent. That
 * total is included in the staff notification email only; it is not
 * yet persisted to the submissions table (a deliberate, separate
 * follow-up step). While pricing is disabled, this behaves exactly
 * as before and no price appears anywhere in the email.
 */
...
// notification email below. If pricing is currently enabled, we
// also add up each option's price_delta - read straight from the
// database, never from anything the browser sent - to arrive at an
// authoritative estimated total for that same email.
$summary_lines    = array();
...
$option_price = $wpdb->get_var( $wpdb->prepare( "SELECT price_delta FROM {$options_table} WHERE id = %d", $option_id ) );

if ( null !== $option_price ) {
$estimated_total += (float) $option_price;
}
}
}
...
self::send_notification_email( $template, $reference_code, $customer_name, $customer_email, $customer_whatsapp, $customer_message, $summary_lines, $pricing_enabled, $estimated_total );
 ...
 *
 * @param bool  $pricing_enabled Whether Settings > BBB Pricing is currently on.
 * @param float $estimated_total Server-calculated total (only meaningful when $pricing_enabled is true).
 */
private static function send_notification_email( $template, $reference_code, $customer_name, $customer_email, $customer_whatsapp, $customer_message, $summary_lines, $pricing_enabled = false, $estimated_total = 0.0 ) {
...
if ( $pricing_enabled ) {
$body .= "\nEstimated Total: AED " . number_format( $estimated_total, 2 ) . "\n";
}
