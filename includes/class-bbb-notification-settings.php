 ...
 * Section 28 (Internal Notifications) and Section 34 (Required
 * Client Inputs) - the shop's Sales Manager and any additional staff
 * who should be emailed whenever a new build request comes in
 * should be configurable by an Administrator, not hardcoded into
 * plugin code.
 *
...
/**
 * Registers the "BBB Notifications" page as a submenu under the
 * main "Bespoke Bike Builder" admin menu, visible only to
 * Administrators.
 */
public static function register_settings_page() {
add_submenu_page( 'bbb-dashboard', 'BBB Notifications', 'Notifications', 'manage_options', 'bbb-notifications', array( __CLASS__, 'render_settings_page' ) );
}

/**
 * Registers the recipients setting with WordPress's Settings API,
 * so it gets saved, sanitised and nonce-protected the standard way
 * without any custom form-handling code in this class.
 */
...
<td>
<input type="text" id="bbb_notification_recipients" name="<?php echo esc_attr( self::OPTION_NAME ); ?>" value="<?php echo esc_attr( self::get_raw_value() ); ?>" class="regular-text" />
